...
(function(global) {
  ...
  $.extend(TreeEditor.prototype, editors.Editor.prototype, {
    render: function(selector) {
      ...
      if (!this.needsRender) { return null; }
      var root = self.value;
    ...
    produce: function(data) {
      ...
      if ( (children && children.length) ||
           (self.leafRelationship &&
            data[self.leafRelationship] &&
            data[self.leafRelationship].length) ) {
        if (children && children.length) {
        ...
        if (self.leafRelationship) {
          ...
          if (leaves && leaves.length) {
            for (var i=0; i < leaves.length; i++) {
              ...
              var leaf = delegate
                    .makeLeaf(self, data[self.leafRelationship][i], self.value)
                    .addClass('leaf');
              delegate
                .addLeafControls(self, leaf, data[self.leafRelationship][i]);
    ...
    addOpenHandler: function(dom, node) {
      var self = this;
      dom.find('.node-name, .icon-collapsed, .icon-expand')
        .off("click")
        .on("click", function(e) {
          e.stopPropagation();
    ...
    forceOpen: function(dom, node) {
      ...
      dom.find("span:first")
        .addClass("icon-expand")
        .removeClass("icon-collapsed").show();
    },
    ...
    addDropHandler: function(dom) {
      ...
      /*
       ...
       from.find(".icon-expand:first").addClass("icon-collapsed")
       .removeClass("icon-expand").hide();
       ...
       // var placeholderNode = self.delegate
       .makePlaceholder(self, droppedEl, onEl);
