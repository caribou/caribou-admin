(function (global) {
  ...
  function AssetFieldEditor( options ) {
  ...
  $.extend( AssetFieldEditor.prototype, editors.PartFieldEditor.prototype, {
    syncToDOM: function() {
      ...
      var slug_class = ".image-" + this.field.slug;
      var selector;
      if ( asset ) {
        var img = $(slug_class);
        if ( asset['content-type'].indexOf("image") >= 0 ) {
          ...
          $(slug_class).attr({ src: "/img/file-icon.png" }).show();
          $(slug_class).after(asset.path);
        }
      } else {
        $(slug_class).hide();
      }
    ...
    attach: function() {
      var self = this;
      var slug = self.model.slug;
      $("#" + slug + "-" + self.field.slug).find("a").click(
        function(e) {
          e.preventDefault();
      ...
      if (this.value.value) {
        var removeLink = $("<a href='#' class='btn btn-primary'>Remove</a>")
              .on("click", function(e) {
                e.preventDefault();
                self.setBlank();
                $(this).remove();
              });
        $("#" + slug + "-" + self.field.slug).find("a").after(removeLink);
      }
    },
    setBlank: function() {
  ...
  $.extend( AssetEditor.prototype, editors.Editor.prototype, {
        ...
        onComplete: function(response) {
          try {
            ...
            if ( self.value['content-type'].indexOf("image") === 0 ) {
              ...
              $("#current-image").hide()
                .after("<b>" + self.value.path + "</b>" );
            }
            self.load(function( data, error, jqxhr ) {
              self.refreshAssets();
            });
          } catch (e) {
            global.caribou.status
              .addErrorMessage("Unable to upload that asset.  Is it too big?")
              .render();
          }
        }
      });
    ...
    loadAssets: function(assets) {
      ...
      _( assets ).each(
        function(a) { self._assetsById[a.id] = a; }
      );
    },
    refreshAssets: function(page) {
      ...
      $.ajax({
        url: self.api().routeFor( "editor-content",
                                  {
                                    model: "asset",
                                    template: "_existing_assets.html",
                                    page: (page || "0"), size: 50
                                  }
                                ),
        type: "GET",
        success: function( data, error, jqxhr ) {
          ...
          $("#assets").find("select[name=images]")
            .imagepicker({ show_label: true });
