(function (global) {
  ...
  $.extend( SpecFieldWhereDelegate.prototype,
            editors.TreeEditorDelegate.prototype,
            {
              ...
              makeNode: function(tree, node) {
                var self = this;
                var dom = $("<li>");
                var slugs = global.caribou.models
                      .unrollFieldSlugs(self.options.editor.spec().model, 2);
                ...
                return dom.append(iconContainer).append(content)
              ...
              makeNodeEditor: function(tree, dom, node, slugs) {
                ...
                var comparatorSelection =
                      $("<select class='spec-comparison-comparator'>");
                ...
                var valueInput =
                      $("<input type='text' class='spec-comparison-value'>");
                ...
                comparisonContainer.append(keySelection)
                  .append(comparatorSelection).append(valueInput);
                ...
                var negateCheckbox =
                      $("<input type='checkbox' class='spec-negate'>");
                ...
                var negateSelection = $("<select class='spec-negate'>")
                      .append("<option value=''>")
              ...
              newControl: function(text, f) {
                var control =
                      $("<a href='#' class='table-link'>" + text + "</a>");
                if (f) {
                  control.on("click", f);
                }
                return control;
              ...
              addClause: function(tree, dom, node) {
                ...
                if (node.children.length === 0) {
                  ...
                  node.negate = false;
                  newDom = tree.produce(node);
                  dom.replaceWith(newDom);
                } else {
                  node.children.push(newNode);
                  newDom = self.makeNode(tree, newNode).addClass("node");
  ...
  $.extend( SpecFieldEditor.prototype, editors.FieldEditor.prototype, {
    ...
    selector: function() {
      return "." + this.model.slug + "-" + this.field.slug;
    },
    ...
    attach: function() {
      ...
      var include =
            $("<h4>including</h4><div class='spec-include'></div>").hide();
      var preview = $("<h4>Preview</h4>" +
                      "<div class='spec-preview-parameters'></div>" +
      ...
      _(global.caribou.api.allModelSlugs()).each(function(slug) {
        modelSelection.append("<option value='" + slug + "'>" +
                              global.caribou.api.model(slug).name +
                              "</option>");
      });
      ...
      var orderAndInclude = $("<div class='well row-fluid spec-order-include'>")
            .append(newOrder).append(newInclude);
      // build editor HTML
      self.element().empty().
                     append("<h4>What</h4>").
    ...
    applyParameters: function(spec) {
      ...
      _(keys).each(function (k) {
        ...
        } else if (newSpec[k] && _.isString(newSpec[k]) &&
                   newSpec[k].match(/^:/)) {
    ...
    makeOrderEditor: function() {
      ...
      var slugs = global.caribou.models
            .unrollFieldSlugs(self.spec().model,
                              2,
                              function(m, f) {
    ...
    makeOrderEditorRow: function(order, slugs) {
      ...
      var directionSelection =
            $("<select class='spec-order-editor-direction'>");
    ...
    makeIncludeEditor: function() {
      ...
      var slugs = global.caribou.models
            .unrollFieldSlugs(self.spec().model, 2, function(m, f) {
              return (m.slug !== "status" &&
                      (f.type === "link" ||
                       f.type === "collection" ||
                       f.type === "part" ));
    ...
    syncFromDOM: function() {
      ...
      self.value.where = self.rollUpWhere(
        self.pruneWhere(self._whereTree.value)
      );
    ...
    unrollWhere: function(where, nodeCounter) {
      ...
      if (keys.length === 1) {
        ...
        if (!SPEC_FIELD_OPERANDS_BY_OPERAND[k]) {
          ...
          if (typeof v === "object") {
            comparator = _(SPEC_FIELD_COMPARATORS).find(function(c) {
    ...
    rollUpWhere: function(w) {
      ...
      var nested;
      if (w.operand === "=") {
        nested = global.caribou.models.dotPathToNestedMap(w.key, w.value);
      ...
      nested = global.caribou.models
        .dotPathToNestedMap(w.key, deepOperandValue);
      _.extend(root, nested);
      //root[w.key] = {};
    ...
    rollUpOrder: function(o) {
      ...
      _(o).each(function(ordering) {
        ...
        if (keys.length === 1 && keys[0]) {
          newOrder.push(global.caribou.models.
                        dotPathToNestedMap(keys[0],
                                           ordering[keys[0]]));
        }
      });
      return newOrder;
    ...
    unrollOrder: function(o) {
      ...
      _(o).each(function(ordering) {
        var unrolled = global.caribou.models.
              nestedMapToDotPathAndValue(ordering);
        var orderMap = {};
    ...
    pruneWhere: function(w) {
      ...
      if (!w.key &&
          !w.value &&
          !SPEC_FIELD_OPERANDS_BY_OPERAND[w.operand]) {
        return null;
      }
      // prune the children
      var children = [];
