(function(global) {
  ...
  NewPageDialog.prototype = {
    ...
    controllerField: function() {
      return this._element.find("select[name=controller]");
    },
    actionField: function() {
      return this._element.find("select[name=action]");
    },
    templateField: function() {
      return this._element.find("input[name=template]");
    },
    controllerOtherField: function() {
      return this._element.find("input[name=controller-other]");
    },
    actionOtherField: function() {
      return this._element.find("input[name=action-other]");
    },
    parentId: function() {
      return this._element.find("input[name=parent-id]").val();
    },
    parentPath: function() {
      return this._element.find("span[id=parent-path]").val();
    },
    ...
    setParentId: function(v) {
      this._element.find("input[name=parent-id]").val(v);
    },
    setParentPath: function(v) {
      this._element.find("span[id=parent-path]").html(v);
    },
    ...
    setControllerOptions: function( data ) {
      var self = this;
      self.controllerField().empty()
        .append("<option value=''>Existing controllers</option>");
      $( data ).each( function( index, item ) {
        var added = "<option value='" +
              item.path +
              "'>" +
              item.path +
              "</option>";
        self.controllerField().append(added);
      });
    ...
    setActionOptions: function( data ) {
      var self = this;
      self.actionField().empty()
        .append("<option value=''>Available actions</options>");
      $( data ).each( function( index, item ) {
        self.actionField()
      ...
      self.actionField().on("change", function() {
        if (!self.template() || !self.templateField().data().dirty) {
          self.setTemplate( self.controller().replace(/\./g, "/") + "/" +
                            self.action().replace(/-/g, "_") + ".html" );
        }
      });
    },
    ...
    $.ajax({
      ...
      success: function( data, error, jqxhr ) {
        ...
        dialog.nameField().on("blur", function(e) {
          if (! dialog.path() ) {
            var new_path = dialog.name().toLowerCase().replace(/[\s_]+/g, "-");
            dialog.setPath( new_path );
          }
        });
        ...
        dialog.actionOtherField().on("blur", function(e) {
          if (! dialog.templateField().data().dirty ) {
            dialog.setTemplate( (dialog.controller() ||
                                 dialog.controllerOther()) +
                                "/" +
                                dialog.actionOther() +
                                ".html" );
          }
  ...
  $.extend( PageTreeDelegate.prototype,
            editors.TreeTableEditorDelegate.prototype,
            { // ack!
              makeHeader: function() {
              ...
              makeNode: function(node) {
                ...
                return $('<tr data-id="' +
                         (node.id) +
                         '" data-parent-id="' +
                         (node.id ? (node.parentId || "0") : "") +
                         '" >' +
                         '<td class="treenode">' + node.label + '</td>' +
                         '<td>' + self.labelFor(node) + '</td>' +
                         '<td>' + (node.node.method || "any") + '</td>' +
                         '<td>' + (node.node.controller || "") + '</td>' +
                         '<td>' + (node.node.action || "") + '</td>' +
                         '<td>' + (node.node.template || "") + '</td>' +
                         '<td class="controls">&nbsp;</td></tr>');
              },
              select: function( pageInfo ) {
                var editorStack = $("#page-editor").data().stack ||
                      $("#page-editor").editorStack();
                editorStack.clear();
                // ack
                var options = {
                  ...
                  submit: function( value, next ) {
                    ...
                    var data = [{
                      model: "page",
                      fields: editor.prepareForUpdate( value )
                    }];
              ...
              addControls: function( tree, el, node ) {
                ...
                if ( node.id && (!node.children ||
                  ...
                  var destroyLink =
                        $("<a href='#' data-model='page' data-id='" +
                          node.id + "'>" +
                          "<span class='instrument-icon-circle-close'></span>" +
  ...
  $.ajax({
    url: global.caribou.api.routeFor("find-all", { model: "page" }),
    success: function( data ) {
